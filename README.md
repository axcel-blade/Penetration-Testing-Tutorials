...
| [Tutorial_8](Tutorial_8/Marigold_Dental_Walkthrough.md) | Marigold Family Dental — a vulnerable appointment-booking target (`setup_marigolddental.sh`) with a three-tier, flag-based walkthrough: a UNION-based SQL injection leaking reused staff credentials, an OpenSSL-encrypted backup decrypted with a passphrase leaked in a neighboring file, and an `LD_PRELOAD` leak in a sudoers rule for privilege escalation |
| [Tutorial_9](Tutorial_9/PixelForge_Print_Shop_Walkthrough.md) | PixelForge Print Shop — a vulnerable order-tracking target (`setup_pixelforge.sh`) with a three-tier, flag-based walkthrough: an OS command injection leaking a staff credential, an unauthenticated Redis job queue leaking a second staff password, and `docker`-group Unix-socket abuse for privilege escalation |
